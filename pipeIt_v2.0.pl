...
for my $pipeType(split(/,/,$pipeList)){
  ...
  $pipeType = 'mm2' if ($pipeType =~ /^(minimap2|minimap|mm2)$/);
	...
	if ($pipeType =~ /^(bwa|bwaaln|mm2|bowtie|ssds|ssdsorig|hic)$/){
	...
	if ($SRAIDs){
		if ($pipeType !~ /(hic|bwa|mm2|ssds|ssdsorig|minimap|ont)/){
			$errMSG .= 'SRA accessions are not yet supported for the '.$pipeType.' pipeline ... '."\n";
		}
	}

	if ($objectStoreRegex){
		if ($pipeType !~ /(hic|ssds|ssdsorig|bwa|mm2|ont|minimap|rtseq|bowtie)/){
