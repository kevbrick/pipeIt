...
$errMSG .= 'No BAM or FASTQ file(s) provided.'."\n" unless ($bam || $fqR1 || $objectStoreRegex || $SRAIDs || $missingBAMok || $dir || $samplesheet);
$errMSG .= "Invalid folder or files specified for sample-sheet [--dir \"$dir\"]"."\n"    if ($dir && !is_valid_Regex($dir,\$ss_data));
$errMSG .= "Invalid sample-sheet [--ss \"$samplesheet\"]"."\n"    if ($samplesheet && !is_valid_Samplesheet($samplesheet,\$ss_data));
...
## ALLOW GC-CORRECTION FILE TO BE PASSED WITH --pipe (e.g. --pipe rtseq:Admera_HiSeqX)
if ($pipeList =~ s/(rtseq):(\S+)/$1/i){
	$gcCorrectionFile = uc($2);
}
  ...
  print SC "rm $outResumeScript \n";
	...
	unless ($dryRun){
		...
		if (!$cmdOK && $inputSize > 30){
			...
			$pipeExec = "sbatch --mail-type=BEGIN,TIME_LIMIT_90,END -J nx.$pipeType.".$randName." --ntasks=".($runLocal?"16":"1")." --mem=".($runLocal?"32":"32")."g --gres=lscratch:800 ".$dependency." --time=".($sBatchTime?$sBatchTime:"72:03:00")." --partition=norm $outScript";
			$cmdOK++;
		}
		...
		if (!$cmdOK && $inputSize > 10){
			...
			$pipeExec = "sbatch --mail-type=BEGIN,TIME_LIMIT_90,END -J nx.$pipeType.".$randName." --ntasks=".($runLocal?"16":"1")." --mem=".($runLocal?"32":"32")."g --gres=lscratch:800 ".$dependency." --time=".($sBatchTime?$sBatchTime:"48:03:00")." --partition=norm $outScript";
			$cmdOK++;
		}
		...
		if (!$cmdOK && $inputSize < 1){
			...
			$pipeExec = "sbatch --mail-type=BEGIN,TIME_LIMIT_90,END -J nx.$pipeType.".$randName." --ntasks=".($runLocal?"16":"1")." --mem=".($runLocal?"16":"32")."g --gres=lscratch:800 ".$dependency." --time=".($sBatchTime?$sBatchTime:"24:03:00")." --partition=norm $outScript";
			$cmdOK++;
		}

		if (!$cmdOK){
			$pipeExec = "sbatch --mail-type=BEGIN,TIME_LIMIT_90,END -J nx.$pipeType.".$randName." --ntasks=".($runLocal?"16":"1")." --mem=".($runLocal?"32":"32")."g --gres=lscratch:200 ".$dependency." --time=".($sBatchTime?$sBatchTime:"24:03:00")." --partition=norm $outScript";
			$cmdOK++;
		}
...
sub whatPipe{ 
  ...
  if ($pipeNF =~ /nf-core/){
		...
		$pArgs .= " --name $outName --outdir $outDir ";
		$pArgs .= " --genome $genome "                                                 if ($type ne "commitfq");
	}
	$pArgs =~ s/\s+(\S)/ $1/g;
